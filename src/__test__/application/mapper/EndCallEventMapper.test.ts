...
describe("EndCallEventMapper - FIMCHAMADA", () => {

    test("Deve processar corretamente um evento de chamada finalizada", () => {
        const rawEvent = {
            regenerated: false,
...
const errors = EventMapper.validate(event);
    ...
    test("Deve retornar erro quando 'attendanceTime' for negativo", () => {
        const rawEvent = {
            ...
            last_sequence: "false",
            parameter2: "10",
            parameter3: "-5" // Erro: Tempo de atendimento negativo
        };
...
const errors = EventMapper.validate(event);
        ...
        expect(event).toEqual({
            ...
            callCenter: true,
            regenerated: false,
            call: {type: 'active', waitingTime: 15, attendanceTime: 45},
            branchesNumber: {destination: ''}
        });
